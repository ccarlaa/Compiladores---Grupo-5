...
void generate_portugol(ASTNode *node)
{
    ...
    case NODE_FUNCTION:
    {
        ...
        // Processa a lista de parâmetros (se houver) que está no filho 1
        if (node->child_count > 1)
        {
            generate_portugol(node->children[1]);
        }

        printf(")\n"); // Fecha os parênteses após processar os parâmetros

        print_indent();
        printf("{\n");
        indent_level++;

        // Processa corpo da função (que está no filho 2)
        ...
        indent_level--;
        print_indent();
        printf("}\n\n"); // Adiciona uma linha em branco para separar funções
        break;
    }

    // caso para a lista de parâmetros
